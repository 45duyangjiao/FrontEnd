$(document).ready(function() {
	...
	$.ajax({
		...
		success:function(data){
			...
			//渲染新闻导航栏
			var str = template("newsNavSub", {
				list: dataNav
			});
			$(".newsNavSub").append(str);
			var dataNavlen=$(".newsNavSub a").length;
		}
	});
...
//			});
			...
			console.log($(".newsNavSub a").length)
			var firstDiv = group.children(":first");
			for(var i = 0,len=dataNav.lenght;i < len; i++){
				var id = dataNav[i].Id;
				if(i == 0){
					firstDiv.attr("id", id);
